...
void TestLinePlot(vtkTelemedVideoSource *TelemedDevice)
{
  ...
  imageToTableAdaptor->SetInputConnection(TelemedDevice->GetOutputPort());
...
int main(int argc, char* argv[])
{
  ...
  if (STRCASECMP(inputConfigFile.c_str(), "")!=0)
  {
    ...
    if ( configRead != NULL )
    {
      TelemedDevice->ReadConfiguration(configRead);
    }
  }

  if(depthCm != -1)
  {
    // TODO : Set the depth (in mm) on the device
  }
  if(gainPercent != -1)
  {
  ...
  if (STRCASECMP(acqMode.c_str(), "B")==0)
  {
    ...
    //TelemedDevice->SetImagingMode(BMode);
    displayMode=SHOW_IMAGE;
  }
  //else if (STRCASECMP(acqMode.c_str(), "RF")==0)
  //{
  //  LOG_DEBUG("Acquisition mode: RF");
  //  //TelemedDevice->SetImagingMode(RfMode);
  //  //TelemedDevice->SetAcquisitionDataType(udtRF);
  ...
  if ( TelemedDevice->Connect()!=PLUS_SUCCESS )
  {
    LOG_ERROR("Unable to connect to Telemed device");
    exit(EXIT_FAILURE);
  }

  TelemedDevice->StartRecording();				//start recording frame from the video
  ...
  if (renderingOff)
  {
    ...
    TelemedDevice->StopRecording();
    TelemedDevice->Disconnect();
  }
  else
  {
    if (displayMode==SHOW_PLOT)
    {
      TestLinePlot(TelemedDevice);
    }
    else
    {
      ...
      viewer->SetInputConnection(TelemedDevice->GetOutputPort());   //set image to the render and window
